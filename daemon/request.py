#
# Copyright (C) 2026 pdnguyen of HCMC University of Technology VNU-HCM.
# All rights reserved.
# This file is part of the CO3093/CO3094 course.
#
# AsynapRous release
#
# The authors hereby grant to Licensee personal permission to use
# and modify the Licensed Source Code for the sole purpose of studying
# while attending the course
#

"""
daemon.request
~~~~~~~~~~~~~~~~~

This module provides a Request object to manage and persist 
request settings (cookies, auth, proxies).
"""
from .dictionary import CaseInsensitiveDict

class Request():
    """The fully mutable "class" `Request <Request>` object,
    containing the exact bytes that will be sent to the server.

    Instances are generated from a "class" `Request <Request>` object, and
    should not be instantiated manually; doing so may produce undesirable
    effects.

    Usage::

      >>> import deamon.request
      >>> req = request.Request()
      ## Incoming message obtain aka. incoming_msg
      >>> r = req.prepare(incoming_msg)
      >>> r
      <Request>
    """
    __attrs__ = [
        "method",
        "url",
        "headers",
        "body",
        "_raw_headers",
        "_raw_body",
        "reason",
        "cookies",
        "routes",
        "hook",
        "user",
        "version",
    ]

    def __init__(self):
        #: HTTP verb to send to the server.
        self.method = None
        #: HTTP URL to send the request to.
        self.url = None
        #: dictionary of HTTP headers.
        self.headers = None
        #: HTTP path
        self.path = None        
        # The cookies set used to create Cookie header
        self.cookies = None
        #: request body to send to the server.
        self.body = None
        # The raw header
        self._raw_headers = None
        #: The raw body
        self._raw_body = None
        #: Routes
        self.routes = {}
        #: Hook point for routed mapped-path
        self.hook = None
        #: User for Authentication
        self.user = None
        #: Version
        self.version = None

    def extract_request_line(self, request):
        try:
            lines = request.splitlines()
            first_line = lines[0]
            method, path, version = first_line.split()

            if path == '/':
                path = '/index.html'
        except Exception:
            return None, None, None

        return method, path, version
             
    def prepare_headers(self, request):
        """Prepares the given HTTP headers."""
        lines = request.splitlines()
        headers = {}
        for line in lines[1:]:
            if ': ' in line:
                key, val = line.split(': ', 1)
                headers[key] = val.strip()
        return headers

    def fetch_headers_body(self, request):
        """Prepares the given HTTP headers."""
        # Split request into header section and body section
        parts = request.split("\r\n\r\n", 1)  # split once at blank line

        _headers = parts[0]
        _body = parts[1] if len(parts) > 1 else ""
        return _headers, _body

    def prepare(self, request, routes=None):
        """Prepares the entire request with the given parameters."""

        # Prepare the request line from the request header
        print("[Request] prepare request missg {}".format(request))
        self.method, self.path, self.version = self.extract_request_line(request)
        print("[Request] {} path {} version {}".format(self.method, self.path, self.version))

        # Keep url/path aligned for downstream Response/Adapter usage.
        self.url = self.path

        #
        # @bksysnet Preapring the webapp hook with AsynapRous instance
        # The default behaviour with HTTP server is empty routed
        #
        # TODO manage the webapp hook in this mounting point
        #
        
        if routes:
            self.routes = routes
            print("[Request] Routing METHOD {} path {}".format(self.method, self.path))
            self.hook = routes.get((self.method, self.path))
            print("[Request] Hook has request {}".format(request))
            #
            # self.hook manipulation goes here
            # ...
            #

        self._raw_headers, self._raw_body = self.fetch_headers_body(request)
        self.headers = CaseInsensitiveDict(self.prepare_headers(self._raw_headers))
        self.body = self._raw_body

        # Parse Cookie header if available.
        cookies = self.headers.get("Cookie", "")
        self.prepare_cookies(cookies)

        return

    def prepare_body(self, data, files, json=None):
        """
        Prepares the request body from data, files, or JSON.

        :param data: Raw body data (str or bytes).
        :param files: File objects (reserved for future use).
        :param json: JSON-serializable object.
        """
        if json is not None:
            import json as json_mod
            self.body = json_mod.dumps(json).encode("utf-8")
        elif isinstance(data, str):
            self.body = data.encode("utf-8")
        elif isinstance(data, bytes):
            self.body = data
        else: 
            self.body = b""
        self.prepare_content_length(self.body)

        return


    def prepare_content_length(self, body):
        """
        Sets the Content-Length header based on the body size.

        :param body: (bytes) The request body.
        """

        if self.headers is None:
            self.headers = CaseInsensitiveDict()
        length = len(body) if body else 0
        self.headers["Content-Length"] = str(length)

        return


    def prepare_auth(self, auth, url=""):
        """
        Prepares the Authorization header from an (username, password) tuple

        :param auth: (tuple) A (username, password) pair.
        :param url: (str) Request URL (reserved for digest auth future use).
        """
        if not auth:
            return
        
        import base64
        username, password = auth
        credentials = f"{username}:{password}"
        encoded = base64.b64encode(credentials.encode("utf-8")).decode("utf-8")

        if self.headers is None:
            self.headers = CaseInsensitiveDict()

        self.headers["Authorization"] = f"Basic {encoded}"

        return

    def prepare_cookies(self, cookies):
        """
        Parses the Cookie header string and stores cookies as a dict.

        :param cookies: (str) Raw Cookie header value 
        """
        if self.headers is None:
            self.headers = CaseInsensitiveDict()

        self.cookies = CaseInsensitiveDict()
        if not cookies:
            return

        for pair in cookies.split(";"):
            pair = pair.strip()
            if "=" in pair:
                key, value = pair.split("=", 1)
                self.cookies[key.strip()] = value.strip()

        if cookies:
            self.headers["Cookie"] = cookies